...
In case a non-trivial table mapping is used in
{func}`reboost.build_hit.build_hit` the TCM to describe the hit files may be
different.
...
An example of a full post-processing of the event tier can be found in
{func}`reboost.build_evt.build_evt`.
